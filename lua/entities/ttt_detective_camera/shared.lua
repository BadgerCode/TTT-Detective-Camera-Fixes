...
if SERVER then
	...
	hook.Add("SetupMove", "TTTCamera.Rotate", function(ply, mv)
		for _, v in ipairs(ents.FindByClass("ttt_detective_camera")) do
			if v.GetPlayer and v.GetShouldPitch then
				if IsValid(v:GetPlayer()) and v:GetPlayer() == ply and v:GetShouldPitch() and ply:Alive() then
...
if CLIENT then
	hook.Add("CreateMove", "TTTCamera.Rotate", function(cmd)
		for _, v in ipairs(ents.FindByClass("ttt_detective_camera")) do
			if v.GetPlayer then
				if IsValid(v:GetPlayer()) and v:GetPlayer() == LocalPlayer() and v:GetShouldPitch() and LocalPlayer():Alive() then
